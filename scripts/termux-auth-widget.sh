...
# CryptoClaw Auth Widget for Termux
...
SERVER="${CRYPTOCLAW_SERVER:-${CLAWDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking CryptoClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/cryptoclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/cryptoclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CRYPTOCLAW_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/cryptoclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CRYPTOCLAW_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: CryptoClaw won't work until re-auth"
                ;;
        esac
        ;;
