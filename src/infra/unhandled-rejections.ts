...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[cryptoclaw] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[cryptoclaw] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[cryptoclaw] FATAL unhandled rejection:", formatUncaughtError(reason));
      process.exit(1);
      return;
    }
    ...
    if (isConfigError(reason)) {
      console.error(
        "[cryptoclaw] CONFIGURATION ERROR - requires fix:",
    ...
    if (isTransientNetworkError(reason)) {
      console.warn(
        "[cryptoclaw] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[cryptoclaw] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
