...
export async function doctorCommand(
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("cryptoclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("cryptoclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("cryptoclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("cryptoclaw doctor --fix")}" to apply changes.`);
  }
