...
async function loadValidConfig() {
  ...
  defaultRuntime.error(`Run \`${formatCliCommand("cryptoclaw doctor")}\` to repair, then retry.`);
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "cryptoclawdocs.termix.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
