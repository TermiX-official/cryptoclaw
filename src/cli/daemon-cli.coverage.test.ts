...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    CRYPTOCLAW_STATE_DIR: process.env.CRYPTOCLAW_STATE_DIR,
    CRYPTOCLAW_CONFIG_PATH: process.env.CRYPTOCLAW_CONFIG_PATH,
    CRYPTOCLAW_GATEWAY_PORT: process.env.CRYPTOCLAW_GATEWAY_PORT,
    CRYPTOCLAW_PROFILE: process.env.CRYPTOCLAW_PROFILE,
  };

  beforeEach(() => {
    process.env.CRYPTOCLAW_STATE_DIR = "/tmp/openclaw-cli-state";
    process.env.CRYPTOCLAW_CONFIG_PATH = "/tmp/openclaw-cli-state/openclaw.json";
    delete process.env.CRYPTOCLAW_GATEWAY_PORT;
    delete process.env.CRYPTOCLAW_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.CRYPTOCLAW_STATE_DIR !== undefined) {
      process.env.CRYPTOCLAW_STATE_DIR = originalEnv.CRYPTOCLAW_STATE_DIR;
    } else {
      delete process.env.CRYPTOCLAW_STATE_DIR;
    }

    if (originalEnv.CRYPTOCLAW_CONFIG_PATH !== undefined) {
      process.env.CRYPTOCLAW_CONFIG_PATH = originalEnv.CRYPTOCLAW_CONFIG_PATH;
    } else {
      delete process.env.CRYPTOCLAW_CONFIG_PATH;
    }

    if (originalEnv.CRYPTOCLAW_GATEWAY_PORT !== undefined) {
      process.env.CRYPTOCLAW_GATEWAY_PORT = originalEnv.CRYPTOCLAW_GATEWAY_PORT;
    } else {
      delete process.env.CRYPTOCLAW_GATEWAY_PORT;
    }

    if (originalEnv.CRYPTOCLAW_PROFILE !== undefined) {
      process.env.CRYPTOCLAW_PROFILE = originalEnv.CRYPTOCLAW_PROFILE;
    } else {
      delete process.env.CRYPTOCLAW_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        CRYPTOCLAW_PROFILE: "dev",
        CRYPTOCLAW_STATE_DIR: "/tmp/openclaw-daemon-state",
        CRYPTOCLAW_CONFIG_PATH: "/tmp/openclaw-daemon-state/openclaw.json",
        CRYPTOCLAW_GATEWAY_PORT: "19001",
      },
