...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.CRYPTOCLAW_STATE_DIR;
  previousConfigPath = process.env.CRYPTOCLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.CRYPTOCLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.CRYPTOCLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.CRYPTOCLAW_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.CRYPTOCLAW_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CRYPTOCLAW_STATE_DIR = path.join(tempHome, ".openclaw");
  delete process.env.CRYPTOCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.CRYPTOCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.CRYPTOCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.CRYPTOCLAW_SKIP_CANVAS_HOST = "1";
  process.env.CRYPTOCLAW_BUNDLED_PLUGINS_DIR = tempHome
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.CRYPTOCLAW_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.CRYPTOCLAW_CONFIG_PATH;
    } else {
      process.env.CRYPTOCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.CRYPTOCLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.CRYPTOCLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.CRYPTOCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CRYPTOCLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.CRYPTOCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.CRYPTOCLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.CRYPTOCLAW_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.CRYPTOCLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.CRYPTOCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CRYPTOCLAW_GATEWAY_TOKEN = fallbackToken;
  }
