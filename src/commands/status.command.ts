...
export async function statusCommand(
  ...
  runtime.log(theme.heading("CryptoClaw status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("cryptoclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("cryptoclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://cryptoclawdocs.termix.ai/faq");
  runtime.log("Troubleshooting: https://cryptoclawdocs.termix.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("cryptoclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("cryptoclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("cryptoclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("cryptoclaw gateway probe")}`);
  }
}
