    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from cryptoclaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      "Docs: https://cryptoclawdocs.termix.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "CryptoClaw update wizard" : "CryptoClaw configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("cryptoclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.CRYPTOCLAW_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.CRYPTOCLAW_GATEWAY_PASSWORD;
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.CRYPTOCLAW_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.CRYPTOCLAW_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.CRYPTOCLAW_GATEWAY_TOKEN;
