...
export async function resolveControlUiDistIndexPath(
  ...
  // Fallback: traverse up and find package.json with name "cryptoclaw" + dist/control-ui/index.html
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgJsonPath) && fs.existsSync(indexPath)) {
      try {
        ...
        if (parsed.name === "cryptoclaw" || parsed.name === "openclaw") {
          return indexPath;
        }
      } catch {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // cryptoclaw.mjs or dist/<bundle>.js
