  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set CRYPTOCLAW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.CRYPTOCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
