...
declare global {
  interface Window {
    __CRYPTOCLAW_ASSISTANT_NAME__?: string;
    __CRYPTOCLAW_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__CRYPTOCLAW_ASSISTANT_NAME__,
    avatar: window.__CRYPTOCLAW_ASSISTANT_AVATAR__,
  });
}
