...
describe("update-cli", () => {
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("CryptoClaw update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    const previousGitDir = process.env.CRYPTOCLAW_GIT_DIR;
    try {
      setTty(true);
      process.env.CRYPTOCLAW_GIT_DIR = tempDir;
      ...
      process.env.CRYPTOCLAW_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
