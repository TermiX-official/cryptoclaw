...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.CRYPTOCLAW_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.CRYPTOCLAW_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.CRYPTOCLAW_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.CRYPTOCLAW_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
