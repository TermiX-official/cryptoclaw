...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.CRYPTOCLAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.CRYPTOCLAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.CRYPTOCLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[cryptoclaw] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[cryptoclaw] ${parsed.error}`);
    process.exit(2);
  }
  ...
  import("./cli/run-main.js")
    ...
    .catch((error) => {
      console.error(
        "[cryptoclaw] Failed to start CLI:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
