...
const shouldBuild = () => {
  if (env.CRYPTOCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.CRYPTOCLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[cryptoclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["cryptoclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
