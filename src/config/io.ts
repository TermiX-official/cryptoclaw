...
function warnIfConfigFromFuture(cfg: OpenClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer CryptoClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
// module scope. `CRYPTOCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CRYPTOCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
