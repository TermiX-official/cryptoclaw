...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CRYPTOCLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.CRYPTOCLAW_STATE_DIR;
  const configPath = env.CRYPTOCLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    CRYPTOCLAW_PROFILE: profile,
    CRYPTOCLAW_STATE_DIR: stateDir,
    CRYPTOCLAW_CONFIG_PATH: configPath,
    CRYPTOCLAW_GATEWAY_PORT: String(port),
    CRYPTOCLAW_GATEWAY_TOKEN: token,
    CRYPTOCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CRYPTOCLAW_SYSTEMD_UNIT: systemdUnit,
    CRYPTOCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CRYPTOCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    CRYPTOCLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    CRYPTOCLAW_STATE_DIR: stateDir,
    CRYPTOCLAW_CONFIG_PATH: configPath,
    CRYPTOCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CRYPTOCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CRYPTOCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CRYPTOCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CRYPTOCLAW_LOG_PREFIX: "node",
    CRYPTOCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CRYPTOCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    CRYPTOCLAW_SERVICE_VERSION: VERSION,
  };
}
