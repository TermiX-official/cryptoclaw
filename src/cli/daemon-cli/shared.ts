...
const SAFE_DAEMON_ENV_KEYS = [
  "CRYPTOCLAW_PROFILE",
  "CRYPTOCLAW_STATE_DIR",
  "CRYPTOCLAW_CONFIG_PATH",
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(
      `Service not installed. Run: ${formatCliCommand("cryptoclaw gateway install", env)}`,
    );
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.CRYPTOCLAW_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("cryptoclaw gateway install", env),
    formatCliCommand("cryptoclaw gateway", env),
  ];
