...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.CRYPTOCLAW_SKIP_BROWSER_CONTROL_SERVER,
  ...
  afterAll(async () => {
    ...
    process.env.CRYPTOCLAW_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.CRYPTOCLAW_STATE_DIR = stateDir;
    process.env.CRYPTOCLAW_CONFIG_PATH = path.join(stateDir, "openclaw.json");
