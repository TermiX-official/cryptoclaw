...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.CRYPTOCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CRYPTOCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CRYPTOCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRYPTOCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
