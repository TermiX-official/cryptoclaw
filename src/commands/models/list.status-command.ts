...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("cryptoclaw models auth setup-token")}\` or \`${formatCliCommand("cryptoclaw configure")}\`.`
          : `Run \`${formatCliCommand("cryptoclaw configure")}\` or set an API key env var.`;
