...
(() => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android CryptoClaw node canvas.");
      return;
    }
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CRYPTOCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.CRYPTOCLAW_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".cryptoclaw", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>CryptoClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
