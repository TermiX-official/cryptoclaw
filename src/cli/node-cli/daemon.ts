...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("cryptoclaw node install"),
    formatCliCommand("cryptoclaw node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("cryptoclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    CRYPTOCLAW_LOG_PREFIX: baseEnv.CRYPTOCLAW_LOG_PREFIX ?? "node",
